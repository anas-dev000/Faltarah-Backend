...
export const getAllServices = async (
  ...
  if (role === "developer")
    return serviceRepo.findAllServices(prisma, null, skip, take);
...
export const getServiceById = async (prisma, id, currentUser) => {
  ...
  let service;
  if (role === "developer")
    service = await serviceRepo.findServiceById(prisma, id);
...
export const createNewService = async (prisma, data, currentUser) => {
  const { role, companyId } = currentUser;
  if (role === "employee")
    throw new AppError("Forbidden: Employees cannot create services", 403);
...
export const updateExistingService = async (prisma, id, data, currentUser) => {
  const { role, companyId } = currentUser;
  if (role === "employee")
    throw new AppError("Forbidden: Employees cannot update services", 403);
  ...
  if (data.name && data.name !== service.name) {
    const nameExists = await serviceRepo.findServiceByName(
      prisma,
      data.name,
      id
    );
